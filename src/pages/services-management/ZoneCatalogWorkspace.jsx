...
export default function ZoneCatalogWorkspace({ zoneId }) {
  const { success, error: toastError } = useToaster();
  const [fromZoneId, setFromZoneId] = useState("");
  const { data: zonesRes } = useGetAllZonesQuery();
  const zones = useMemo(() => zonesArrayFromGetZonesResponse(zonesRes), [zonesRes]);
  const { data, isLoading, isError, error: catalogError, refetch } = useGetZoneCatalogQuery(
    zoneId,
    { skip: !zoneId }
  );
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      success("Copied overlays from selected zone");
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ ...CARD, display: "flex", flexWrap: "wrap", gap: 12, alignItems: "end" }}>
        ...
        <Button variant="secondary" disabled={!fromZoneId || copying} onClick={handleCopy}>
      ...
      {isLoading ? (
        ...
        <div style={CARD}>
          ...
          <p className="jd-lead" style={{ marginTop: 8 }}>
            {catalogError?.data?.message || "Request failed for this zone."}
          </p>
        ...
        <>
          <p className="jd-lead" style={{ marginBottom: 12 }}>
            Zone mode is active. Any change here writes zone overlays only. Master catalog stays unchanged.
          </p>
            ...
            <div key={svc.serviceId} style={CARD}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#6b7280" }}>
                    {svc.inherited ? "Inherited" : "Overridden"} | {svc.isEnabled ? "Visible" : "Hidden"}
                  </div>
                </div>
                <Button
                  size="sm"
              ...
              {(svc.preferences || []).length > 0 ? (
                <div style={{ marginTop: 12 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Preferences</div>
                  <PreferenceRows
                    serviceId={svc.serviceId}
                    rows={svc.preferences || []}
                    disabled={busy}
                    onSave={save}
                    onReset={resetRow}
                  />
                ...
                <div key={cat.categoryId} style={{ marginTop: 12 }}>
                  ...
                  {(cat.items || []).map((item) => (
                    <ItemRow
                      ...
                      onReset={() => resetRow({ type: "item", entityId: item.subCategoryId })}
                ...
                <div key={cat.addOnCategoryId} style={{ marginTop: 12 }}>
                  ...
                  {(cat.addOns || []).map((addon) => (
                    <PriceRow
                      ...
                      onReset={() => resetRow({ type: "addOn", entityId: addon.addOnServiceId })}
                    />
                  ))}
                </div>
              ))}
...
function PreferenceRows({ serviceId, rows, disabled, onSave, onReset, depth = 0 }) {
  return (
    <div style={{ display: "grid", gap: 6 }}>
      {(rows || []).map((pref) => (
        <PreferenceRow
          key={`${serviceId}-${pref.id}-${pref.sortOrder}-${pref.isEnabled}`}
          pref={pref}
          serviceId={serviceId}
          disabled={disabled}
          onSave={onSave}
...
function PreferenceRow({ pref, serviceId, disabled, onSave, onReset, depth }) {
  const [sortOrder, setSortOrder] = useState(
    pref.sortOrder == null ? "" : String(pref.sortOrder)
  );
  return (
    <div style={{ borderBottom: "1px solid #f1f4f8", paddingBottom: 6 }}>
      ...
      >
        <div style={{ flex: "1 1 220px" }}>
          <div style={{ fontSize: 13 }}>{pref.name}</div>
          <div style={{ fontSize: 11, color: "#6b7280" }}>
            {pref.inherited ? "Inherited" : "Overridden"} | {pref.isEnabled ? "Visible" : "Hidden"}
          </div>
        </div>
        <Input
          type="number"
        ...
        <Button
          ...
          onClick={() =>
            onSave(
              ...
              pref.isEnabled ? "Preference hidden" : "Preference shown"
            )
          }
        >
          {pref.isEnabled ? "Hide" : "Show"}
        </Button>
        {!pref.inherited ? (
          <Button
            size="sm"
      ...
      {(pref.childTypes || []).length > 0 ? (
        <PreferenceRows
          serviceId={serviceId}
          rows={pref.childTypes}
          disabled={disabled}
          onSave={onSave}
          onReset={onReset}
          depth={depth + 1}
    ...
    >
      ...
      >
        <div style={{ flex: "1 1 160px" }}>
          ...
          <div style={{ fontSize: 11, color: "#6b7280" }}>
            {item.staged ? "Staged override (flag off)" : item.priceInherited ? "Inherited master" : "Zone override"}
  ...
  return (
    ...
    >
      <div style={{ flex: "1 1 160px" }}>
        ...
        <div style={{ fontSize: 11, color: "#6b7280" }}>
          {staged ? "Staged override (flag off)" : inherited ? "Inherited master" : "Zone override"}
          {hidden ? " | Hidden" : ""}
        </div>
      </div>
      <Input
