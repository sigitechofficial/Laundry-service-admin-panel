...
export default function CancelledOrders() {
  ...
  const pickRows = useCallback((res) => res?.data?.cancelOrders, []);
  const {
    statsQueryParams,
    rows: orderBookings,
    totalRows,
    ...
    pickRows,
    totalCountField: "cancelBookingCount",
  });
  ...
  const [fetchCancelledOrders] = useLazyGetCancelledOrdersQuery();
  const csv = useOrderListCsvExport({
    tableFilters,
    fetchList: fetchCancelledOrders,
    pickRows,
    filenameBase: "orders-cancelled",
    orderStatuses,
  });
    ...
    <>
      <div className="min-w-0">
        <OrderPageHeader
          ...
          description="Audit cancelled orders while retaining their service, schedule and monetary context."
        />

        {isError ? (
          <OrderError>
            Could not load cancelled orders. Adjust filters or refresh the page.
          </OrderError>
        ) : null}

        <OrderMetrics
          items={tabOrderMetricItems({
            tabLabel: "Cancelled orders",
            tabValue: totalRows,
            tabTone: "danger",
